"""
Empirical July 1 - September 16, 2026 Comprehensive Performance Audit for All 6 Strategies.
Generates Trade Ledger CSV, Strategy Summary CSV, and 4-Panel Graphical Dashboard.
"""
import os
import sys
import shutil
...
period_end = pd.Timestamp("2026-09-16")
period_df = df[(df["datetime"] >= period_start) & (df["datetime"] <= period_end)].copy()
period_dates = sorted(period_df["datetime"].tolist())

trade_ledger = []

# -------------------------------------------------------------
# Strategy 1: Apex VRP Engine (NIFTY Weekly Iron Condors)
# Capital: Rs 1,00,000 (Required SPAN margin ~Rs 75,000)
# -------------------------------------------------------------
s1_cap = 100000.0
# Monday or start of week entry
for idx in period_df.index:
    row = df.iloc[idx]
    dt = row["datetime"]
    vix = row["vix"]
    nc = row["close"]
    # Check if Monday or first day of period
    if dt.weekday() == 0 or dt == period_start:
        if vix < 20.0:
            ...
            trade_ledger.append({
                ...
                "Details": f"Spot {nc:.0f} | Put {put_k:.0f} / Call {call_k:.0f} (Range {min_l:.0f}-{max_h:.0f})"
            })

# -------------------------------------------------------------
# Strategy 2: Zen Curvature Spread (Put Credit Spread)
# Capital: Rs 1,00,000 (Required SPAN margin ~Rs 65,000)
# -------------------------------------------------------------
s2_cap = 100000.0
for i in range(period_df.index[0], len(df) - 1, 2):
    row = df.iloc[i]
    dt = row["datetime"]
    if dt > period_end:
        break
...
# -------------------------------------------------------------
s3_cap = 20000.0
s3_engine = ConfluenceGammaScalperStrategy(initial_capital=s3_cap)
s3_sim = s3_engine.run_simulation(df)
for t in s3_sim.get("trades", []):
    t_dt = pd.to_datetime(t["date"])
    if period_start <= t_dt <= period_end:
        trade_ledger.append({
            "Date": t_dt.strftime("%Y-%m-%d"),
            "Strategy": "Strategy 3: Confluence Gamma Scalper",
            "Instrument": "NIFTY 25 Qty Option Buyer",
            "Assigned_Capital": s3_cap,
            "Position": f"Buy {t['option_type']}",
            "Gross_PnL": round(t["gross_pnl"], 2),
            "Friction_Taxes": round(t["gross_pnl"] - t["net_pnl"], 2),
            "Net_PnL": round(t["net_pnl"], 2),
            "Win": t["win"],
            "Exit_Reason": t["exit_reason"],
            "Details": f"Spot Entry {t['spot_entry']:.1f} -> Exit {t['spot_exit']:.1f}"
        })
...
for i in range(df_gt[df_gt["datetime"] >= period_start].index[0], len(df_gt)):
    row = df_gt.iloc[i]
    if row["datetime"] > period_end:
        break
    prev = df_gt.iloc[i - 1]
    atr = prev["atr_14"]
    sig = 0
...
# -------------------------------------------------------------
# Strategy 5: Velocity-5 Scalper
# Capital: Rs 20,000
# -------------------------------------------------------------
s5_cap = 20000.0
s5_engine = ActiveMomentumOptionScalperStrategy(initial_capital=s5_cap)
s5_sim = s5_engine.run_simulation(df)
for t in s5_sim.get("trades", []):
    t_dt = pd.to_datetime(t["date"])
    if period_start <= t_dt <= period_end:
        trade_ledger.append({
            "Date": t_dt.strftime("%Y-%m-%d"),
            "Strategy": "Strategy 5: Velocity-5 Scalper",
            "Instrument": "NIFTY 25 Qty Option Buyer",
            "Assigned_Capital": s5_cap,
            "Position": f"Buy {t['option_type']}",
            "Gross_PnL": round(t["gross_pnl"], 2),
            "Friction_Taxes": round(t["gross_pnl"] - t["net_pnl"], 2),
            "Net_PnL": round(t["net_pnl"], 2),
...
# Capital: Rs 10,000 (also tracked for Rs 20,000)
# -------------------------------------------------------------
s6_cap = 10000.0
...
for i in range(df_eval[df_eval["datetime"] >= period_start].index[0], len(df_eval)):
    row = df_eval.iloc[i]
    if row["datetime"] > period_end:
        break
...
ledger_csv_path = "reports/real_2026/july_to_september_2026_trade_ledger.csv"
ledger_df.to_csv(ledger_csv_path, index=False)
print(f"Saved: {ledger_csv_path} ({len(ledger_df)} total trades)")
...
summary_csv_path = "reports/real_2026/july_to_september_2026_strategy_summary.csv"
summary_df.to_csv(summary_csv_path, index=False)
print(f"Saved: {summary_csv_path}")

# -------------------------------------------------------------
# Generate High-Res 4-Panel Chart (July 1 - Sep 16)
# -------------------------------------------------------------
os.makedirs("reports/charts", exist_ok=True)
chart_path = "reports/charts/july_to_september_2026_performance.png"

plt.style.use("dark_background")
fig, axes = plt.subplots(2, 2, figsize=(16, 11), dpi=180)
fig.suptitle("NSE NIFTY 50 Strategy Performance Audit: July 1 – September 16, 2026", fontsize=18, fontweight="bold", y=0.98, color="#ECEFF4")

all_dates = [d.strftime("%Y-%m-%d") for d in period_dates]

# --- Plot 1: Cumulative Net Realized Equity Curves ---
ax1 = axes[0, 0]
ax1.set_title("1. Cumulative Realized Net P&L (₹)", fontsize=13, fontweight="bold", color="#88C0D0", pad=10)
...
for name, color in colors.items():
    ...
    ax1.plot(all_dates, cum_pnl, label=name.split(":")[1].strip(), color=color, linewidth=2.3)
...
# Show ticks every 7 trading days
tick_indices = list(range(0, len(all_dates), 7))
if (len(all_dates) - 1) not in tick_indices:
    tick_indices.append(len(all_dates) - 1)
ax1.set_xticks(tick_indices)
ax1.set_xticklabels([all_dates[i] for i in tick_indices], rotation=35, fontsize=8.5)

# --- Plot 2: Return on Assigned Capital (%) Bar Chart ---
ax2 = axes[0, 1]
ax2.set_title("2. Return on Capital (ROC %)", fontsize=13, fontweight="bold", color="#88C0D0", pad=10)
...
# --- Plot 3: NIFTY 50 Macro Context (July - Sep) ---
ax3 = axes[1, 0]
ax3.set_title("3. NIFTY 50 Index Trend & Regime (55 Trading Days)", fontsize=13, fontweight="bold", color="#88C0D0", pad=10)

ax3.plot(all_dates, period_df["close"], color="#D8DEE9", linewidth=2.0, label="NIFTY 50 Close")
ax3.fill_between(all_dates, period_df["low"], period_df["high"], color="#434C5E", alpha=0.35, label="High-Low Range")
...
ax3.set_xticklabels([all_dates[i] for i in tick_indices], rotation=35, fontsize=8.5)

# --- Plot 4: Gross P&L vs Total Indian Regulatory & Broker Friction ---
ax4 = axes[1, 1]
ax4.set_title("4. Gross Edge vs Regulatory & Broker Friction (₹)", fontsize=13, fontweight="bold", color="#88C0D0", pad=10)
...
if conv_artifact_dir.exists():
    shutil.copy(chart_path, conv_artifact_dir / "july_to_september_2026_performance.png")
    shutil.copy(ledger_csv_path, conv_artifact_dir / "july_to_september_2026_trade_ledger.csv")
    shutil.copy(summary_csv_path, conv_artifact_dir / "july_to_september_2026_strategy_summary.csv")
