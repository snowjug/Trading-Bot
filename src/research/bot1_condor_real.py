...
def build_condor(
    ...
    chain = store[(store["TradDt"] == entry_day) & (store["XpryDt"] == expiry)]
    if chain.empty:
...
def run_real_condor_backtest(
    ...
    for i in range(len(d)):
        ...
        built = build_condor(store, sess, expiry, spot, float(vix), otm_sd, wing_sd)
        if not built["ok"]:
            rej(built["reason"]); continue

        settle = settlement_price(store, expiry, index_close)
        if settle is None:
            rej("NO_SETTLEMENT_PRICE"); continue

        legs: List[CondorLeg] = []
        gross = 0.0
        lot = int(built["legs"][0][4]["NewBrdLotQty"])
        ...
        trades.append(CondorTrade(
            ...
            lot_size=lot, settlement_source=settlement_source(store, expiry),
        ))
...
def adverse_fill_pnl(trade: "CondorTrade") -> Optional[Dict[str, Any]]:
    ...
    with real exchange prints instead of an assumed spread, which matters because
    the credit is only ~12 points spread across four legs.
